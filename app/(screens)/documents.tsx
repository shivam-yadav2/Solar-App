...
export default function DocumentsScreen() {
  ...
  return <Screen title="Documents" subtitle="Project files and certificates" onBack={() => router.back()} action={hasPermission('documents.create') ? <HeaderButton onPress={() => router.push('/document-form')} /> : undefined} refreshing={state.isRefreshing} onRefresh={state.refresh}>
    {state.isLoading ? <Loading /> : state.error ? <ErrorState message={state.error} onRetry={state.reload} /> : items.length === 0 ? <EmptyState message="No documents uploaded yet." /> : items.map((item) => <Pressable key={item.id} onPress={async () => { if (!item.fileId) return; const url = await api.getFileUrl(item.fileId); await Linking.openURL(url); }}><Card>
